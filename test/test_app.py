...
class TestApplicationWhenAuthorized:
    def test_app_should_return_code_200(self, create_user, test_client):
        _authentication_headers = get_authentication_headers(g.test_user_username, g.test_user_password)
        response = test_client.get('/', headers=_authentication_headers)
        assert response.status_code == HttpStatus.HTTP_200_OK

    def test_app_should_return_data_in_json(self, create_user, test_client):
        _authentication_headers = get_authentication_headers(g.test_user_username, g.test_user_password)
        response = test_client.get('/', headers=_authentication_headers)
        assert_header_value('Content-Type', 'application/json', response.headers)

    def test_app_should_return_hello_world(self, create_user, test_client):
        _authentication_headers = get_authentication_headers(g.test_user_username, g.test_user_password)
        response = test_client.get('/', headers=_authentication_headers)
