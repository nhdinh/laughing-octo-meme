...
class TestApplicationWhenAuthorized:
    def test_app_should_return_code_200(self, create_authorization_headers, test_client):
        response = test_client.get('/', headers=create_authorization_headers)
        assert response.status_code == HttpStatus.HTTP_200_OK

    def test_app_should_return_data_in_json(self, create_authorization_headers, test_client):
        response = test_client.get('/', headers=create_authorization_headers)
        assert_header_value('Content-Type', 'application/json', response.headers)

    def test_app_should_return_hello_world(self, create_authorization_headers, test_client):
        response = test_client.get('/', headers=create_authorization_headers)
